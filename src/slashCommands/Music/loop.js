       ...
       const input = interaction.options.getString("input");
        ...
        if(input === "track") {
            ...
            return await interaction.editReply({
              embeds: [new MessageEmbed().setColor('#ff0000').setDescription(`<a:tick:919227918738133022>** | Loop track is now disable**`)]})   
            } else {
            player.setTrackRepeat(true);
            return await interaction.editReply({
              embeds: [new MessageEmbed().setColor('#ff0000').setDescription(`<a:tick:919227918738133022>** | Loop track is now enable**`)]})
          }
        } else if(input === "queue") {
            if(!player.queue.size) return await interaction.editReply({
              embeds: [new MessageEmbed().setColor('#ff0000').setDescription(`No more songs left in the queue to loop`)]})
         if(player.queueRepeat) {
                player.setQueueRepeat(false);
            return await interaction.editReply({
              embeds: [new MessageEmbed().setColor('#ff0000').setDescription(`<a:tick:919227918738133022>** | Loop queue is now disable**`)]})
          } else {
          player.setQueueRepeat(true);
          return await interaction.editReply({
              embeds: [new MessageEmbed().setColor('#ff0000').setDescription(`<a:tick:919227918738133022>** | Loop queue is now enable**`)]})
          };
       }
    }
};
