...
module.exports = {
    ...
    if(!player) return await interaction.editReply({
        ...
        if(!player.queue.size || player.queue.size === 0) {
            ...
            const embed = new MessageEmbed().setColor('#ff0000').setDescription(`<a:playing:919227920260694087>** | Now Playing** [${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${pms(player.position)} / ${pms(player.queue.current.duration)} ]\` • [${player.queue.current.requester}]`);
            ...
            if(player.queue.size < 10 || player.queue.totalSize < 10) {
                ...
                const embed2 = new MessageEmbed().setTitle(`${interaction.guild.name} Server Queue`).setColor('#ff0000').setDescription(`<a:playing:919227920260694087>** | Now Playing**\n[${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${pms(player.position)} / ${pms(player.queue.current.duration)} ]\` • [${player.queue.current.requester}]\n\n**Queued Songs**\n${pages[page]}`).setFooter(`Page ${page + 1}/${pages.length}`, interaction.user.displayAvatarURL({ dynamic: true })).setThumbnail(player.queue.current.thumbnail).setTimestamp()

                await interaction.editReply({
                    embeds: [embed2]
                }).catch(() => {});
            } else {
                const embed3 = new MessageEmbed().setTitle(`${interaction.guild.name} Server Queue`).setColor('#ff0000').setDescription(`<a:playing:919227920260694087>** | Now Playing**\n[${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${pms(player.position)} / ${pms(player.queue.current.duration)} ]\` • [${player.queue.current.requester}]\n\n**Queued Songs**\n${pages[page]}`).setFooter(`Page ${page + 1}/${pages.length}`, interaction.user.displayAvatarURL({ dynamic: true })).setThumbnail(player.queue.current.thumbnail).setTimestamp()
                ...
                collector.on("collect", async (button) => {
                    if(button.customId === "queue_cmd_but_1_app") {
                        ...
                        const embed5 = new MessageEmbed().setColor('#ff0000').setDescription(`<a:playing:919227920260694087>** | Now Playing**\n[${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${pms(player.position)} / ${pms(player.queue.current.duration)} ]\` • [${player.queue.current.requester}]\n\n**Queued Songs**\n${pages[page]}`).setFooter(`Page ${page + 1}/${pages.length}`, button.user.displayAvatarURL({ dynamic: true })).setThumbnail(player.queue.current.thumbnail).setTimestamp()
