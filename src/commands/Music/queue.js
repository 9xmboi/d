...
module.exports = {
   ...
   execute: async (message, args, client, prefix) => {
            ...
            if(!player.queue) return message.channel.send({ embeds: [new MessageEmbed().setColor('#ff0000').setTimestamp().setDescription(`Nothing is playing right now.`)]});
           
            if(player.queue.length === "0" || !player.queue.length) {
                const embed = new MessageEmbed()
                .setColor('#ff0000')
                .setDescription(`<a:playing:919227920260694087>** | Now Playing** [${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${pms(player.position)} / ${pms(player.queue.current.duration)} ]\` • [${player.queue.current.requester}]`)
